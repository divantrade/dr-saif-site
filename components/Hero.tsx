...
/**
 * Editorial hero in a 2/3 + 1/3 split on lg+:
 *   Right (visually, in RTL): identity column — name, role, project
 *     statement and search, paired with the portrait in a dark panel.
 *   Left (visually): an independent rotating panel that crossfades
 *     through four short quotes from the doctor, auto-advancing every
 *     six seconds, with dot navigation. Pauses on hover/focus.
 *
 * On smaller screens the three pieces stack vertically:
 *   text → portrait → carousel.
 */
  ...
  return (
    <section className="relative bg-[#faf7f1] text-gray-900 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-20 lg:py-24">
        {/* Outer 12-col grid: identity 8 cols + carousel 4 cols on lg+ */}
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-10 items-stretch">
          {/* ─── Identity column (2/3) ───────────────────────────────── */}
          <div className="lg:col-span-8 grid lg:grid-cols-7 gap-8 lg:gap-10 items-stretch">
            {/* Text */}
            <div className="lg:col-span-4 order-1 flex flex-col justify-center">
              <h1 className="font-display font-bold leading-[0.95] tracking-tight mb-8">
                <span className="block text-5xl sm:text-6xl lg:text-6xl xl:text-7xl text-stone-900">
                  سيف الدين
                </span>
                <span className="block text-5xl sm:text-6xl lg:text-6xl xl:text-7xl text-emerald-900 mt-2">
                  عبد الفتاح
                </span>
                <span className="block text-lg sm:text-xl lg:text-xl xl:text-2xl text-stone-500 mt-5 font-normal italic">
                  أستاذ العلوم السياسية — جامعة القاهرة
                </span>
              </h1>

              <p className="text-base md:text-lg text-stone-700 leading-[2] max-w-xl mb-8">
                مشروع فكري في
                ...
                {" — "}
                يقرأ الواقع من منظور منهاجي مقاوم، و يُعيد السؤال قبل
                أن يجترح الجواب.
              </p>
            ...
            {/* Portrait — same dramatic dark panel as before, tighter
                column so it sits comfortably beside the new carousel. */}
            <div className="lg:col-span-3 order-2 relative">
